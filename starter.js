...
const calculator = (num1, num2, callback) => {
    if(+num1 && +num2){
        ...
        console.log('UR trash kid')
    }
}

// console.log(calculator(420, 69, multiply))
...
const applyDiscountsByCatergory = (arr, category, callback, discount) => {
    arr.forEach(product => {
...
applyDiscountsByCatergory(dogProducts, 2, applyFlatRateDiscount, 1)
...
// const makeWheatSandwhichFunction = makeSandwich('wheat')
// const makeRyeSandwhich = makeSandwich('Rye')

// console.log(makeWheatSandwhichFunction(['tomato', 'lettuce', 'bacon']))
// console.log(makeRyeSandwhich(['mayo']))
...
//this function tales a string and formats it in camel case
const copyStrToCamelCase = str => {
    //str is just a string with possible spaces in it, i want that split in an array of the words it contains
    const splitStr = str.split(' ')
    ...
    for (let i = 0; i < splitStr.length; i++) {
        //set a variable to a current word we're looking at in the loop
        let word = splitStr[i]

        //take that word and lowercase all of it
        word = word.toLowerCase()

        //if the word wasn;t the first one in the string, then we need to capitilize it
        if (i !== 0) {
            word = word.charAt(0).toUpperCase() + word.slice(1)
        }
